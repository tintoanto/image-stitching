function main()
    IMatrix = preprocess(4);           % selecting folder and setting all images to IMatrix
...
%     developed as part of HW 2.
    ...
    r= {}; c= {};
    for idx = 1:num
        [~, ridx, cidx]  = harris(IMatrix(:,:,idx), sigma, threshold, radius, display);   
...
%     something like this (but hopefully with a more precise alignment):
    ...
    im2t=imtransform(IMatrix(:,:,2),T,'XData',xdataout,'YData',ydataout);
    im1t=imtransform(IMatrix(:,:,1),maketform('affine',eye(3)),'XData',xdataout,'YData',ydataout);
